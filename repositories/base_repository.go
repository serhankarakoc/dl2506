...
func (r *BaseRepository[T]) Delete(ctx context.Context, id uint) error {
	...
	if !ok || userID == 0 { return ErrMissingUserID }
	tx := r.db.WithContext(ctx)
	if err := tx.First(&entity, id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) { return ErrNotFound }
		return err
	}
	if err := tx.Model(&entity).Update("deleted_by", userID).Error; err != nil { return err }
...
func (r *BaseRepository[T]) BulkDelete(ctx context.Context, condition map[string]interface{}) error {
	...
	if !ok || userID == 0 { return ErrMissingUserID }
	tx := r.db.WithContext(ctx)
	if err := tx.Where(condition).Find(&entities).Error; err != nil { return err }
	for _, entity := range entities {
		if err := tx.Model(&entity).Update("deleted_by", userID).Error; err != nil { return err }
		if err := tx.Delete(&entity).Error; err != nil { return err }
	}
	return nil
}
...
func (r *BaseRepository[T]) BulkDeleteWithRelations(ctx context.Context, ids []uint) error {
	...
	if !ok || userID == 0 { return ErrMissingUserID }
	tx := r.db.WithContext(ctx)
	if err := tx.Preload(clause.Associations).Find(&entities, ids).Error; err != nil { return err }
	for _, entity := range entities {
		if err := tx.Model(&entity).Update("deleted_by", userID).Error; err != nil { return err }
	}
